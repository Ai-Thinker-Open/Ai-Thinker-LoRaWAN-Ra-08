...
static TimerEvent_t SHT3x_Timer;
static void SHT3x_timer_event(void)
{
    int32_t i32_temp;
    ...
    TimerStop(&SHT3x_Timer);
    sht3x_ReadTempHumi(&i32_temp, &u32_tempPoint, &u32_humi);
    snprintf(payload, 64,"temp:%d.%02u,humi:%02u",i32_temp, u32_tempPoint, u32_humi);
    printf("%s\r\n", payload);

    if (lwan_data_send(0, 3, payload, strlen(payload)) == LWAN_SUCCESS) {
        printf("\r\nOK+SEND:%02d\r\n", strlen(payload));
    }else{
        printf("\r\nERR+SEND:00\r\n");
    }
    ...
    // TimerSetValue(&SHT3x_Timer,3000);
    TimerStart(&SHT3x_Timer); 
}
...
#error "Please define a region in the compiler options."
            ...
            case DEVICE_STATE_JOINED: {
                ...
                //SHT3x timer
                {
                    printf("start tensor SHT3x upload timer:%d S\r\n", 5);
                    TimerInit(&SHT3x_Timer, SHT3x_timer_event);
                    TimerSetValue(&SHT3x_Timer,5000);
                    TimerStart(&SHT3x_Timer);  
                }
                break;
            }
